...
void InitInstance(){
	...
	portals.color_2d.red = GetRValue( color_2d ) / 255.0f;
	portals.color_2d.green = GetGValue( color_2d ) / 255.0f;
	portals.color_2d.blue = GetBValue( color_2d ) / 255.0f;
	...
	portals.color_3d.red = GetRValue( color_3d ) / 255.0f;
	portals.color_3d.green = GetGValue( color_3d ) / 255.0f;
	portals.color_3d.blue = GetBValue( color_3d ) / 255.0f;
	portals.color_3d.alpha = 1;

	COLORREF color_fog = (COLORREF)INIGetInt( COLOR_FOG, RGB( 127, 127, 127 ) ) & 0xFFFFFF;
	portals.color_fog.red = GetRValue( color_fog ) / 255.0f;
	portals.color_fog.green = GetGValue( color_fog ) / 255.0f;
	portals.color_fog.blue = GetBValue( color_fog ) / 255.0f;
